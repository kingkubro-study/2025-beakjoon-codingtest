/** 백준 20168: 골목 대장 호석 - 기능성 (다익스트라 응용/골드5)
 *
 * N개 교차로, M개 골목
...
// output(int):  최소 요금
// 퓰이2: 다익스트라+이진탐색 & 우선순위큐
// 메모리 9652 KB,  시간 96 ms
function dijsktra(A, B, C, mid, graph, N) {
  // 다익스트라 알고리즘 호출시마다 비용 배열 새로 초기화
  // INF로 채우기
  ...
  // 목적지 B까지 요금 C 이하로 갈 수 있는 지
  return cost[B] <= C;
}

function solution(N, M, A, B, C, dataList) {
  const INF = Number.MAX_SAFE_INTEGER;
  let answer = INF; // 최단 거리 마지막에 바꿀거니깐 let으로 선언
  ...
  // 비용 이분탐색
  let lt = 0;
  let rt = cList.length - 1;
  cList.sort((a, b) => a - b);
  while (lt <= rt) {
    const mid = Math.floor((lt + rt) / 2);

    // 중간 비용을 최대로 갖는 경로 찾기
    if (dijsktra(A, B, C, cList[mid], graph, N)) {
      // 최대 줄이기
      rt = mid - 1;
      // answer 업데이트
      answer = Math.min(answer, cList[mid]);
    } else {
      // 최소 늘리기
      lt = mid + 1;
    }
...
// 퓰이1: 우선순위 큐 & 다익스트라,	 메모리 9432 KB,  시간 96 ms
// 맞았습니다(10/18) 노란색
/*
function solution(N, M, A, B, C, dataList) {
